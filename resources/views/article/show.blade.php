...
@section('content')
    ...
    <article class=" h-full w-full flex flex-col items-center justify-center gap-6 ">
        <div
            class="relative h-96 w-96 px-10 py-8 flex flex-col items-left justify-between gap-1 bg-[#F5F4FE] shadow-xl rounded-3xl">
            <div
            ...
            <div>
                <h1><small>@lang('lang.title'): <br> </small>{{ ucfirst($article->title) }}</h1>
            </div>
            <div>
                <p><small>@lang('lang.content'): <br> </small>{{ ucfirst($article->body) }}</p>
            </div>
            <p><small>@lang('lang.written_by'): <br> </small> {{ $article->nom }} {{ $article->prenom }}</p>
